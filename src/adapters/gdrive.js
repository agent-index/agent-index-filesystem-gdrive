...
export class GoogleDriveAdapter {
  ...
  async writeBatch(entries, options = {}) {
    ...
    // ── Step 1: ensure unique parent dirs ONCE, single-threaded, under lock ──
    // (duplicate-parent-folder guard — bug bulkuploadserial / concurrency review)
    ...
    // ── Step 2: write each file (parents now exist + are cached) ──
    const results = [];
  ...
  async statBatch(paths) {
    ...
    for (const p of paths) {
      try {
        ...
        if (!id) { results.push({ path: p, exists: false }); continue; }
